...
bool errorCheck(QString& s)
{
    int i=0;
    int bracketMatch=0;
    QString s_operator="+-*/\^sincoslogtan. ()（）[]{}dot%";
    QString bySymbol = "×";
    QString divideSymbol = "÷";
    if(s[s.size()-1]=='=')
        s.remove(s.size()-1,1);         //因为习惯可能在表达式最后面家里个等号
    for(i=0;i<s.size();i++)
    {
        if(s[i]=='\\'||s[i]=='%'||s[i]==divideSymbol[0])    //暂不支持取余操作，%算为除
            s[i]='/';
        if(s[i]==bySymbol[0])
            s[i]='*';
        if(s[i]=='['||s[i]=='[')
...
QString infixToSuffix(const QString &s)
{
...
double calSuffix(const QString &s)
{
    stack<double> result;
    double temp;
    QString num="";
...
double cal_E_Formal(const QString &s)
{
